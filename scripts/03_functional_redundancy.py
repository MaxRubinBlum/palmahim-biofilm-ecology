#!/usr/bin/env python3
"""Functional redundancy/core-guild table for Supplementary Table S5.
...
def main():
    ...
    A = read(args.abundance).astype(float)
    T = read(args.traits)
    tax = read(args.taxonomy)
    ...
    mags = [m for m in A.columns if m in T.index and m in tax.index]
    A = A[mags]
    T = T.loc[mags]
    tax = tax.loc[mags]
    ...
    for trait in T.columns:
        ...
        summed = X.sum(axis=1)
        eff = [effective_mag_number(r) for r in X]
        rows.append({
            "function": trait,
            "carrier_MAGs": len(cmags),
            "carrier_orders": tax.loc[cmags, "Order"].nunique() if cmags else 0,
            "min_abundance": summed.min() if len(summed) else np.nan,
            "median_abundance": np.median(summed) if len(summed) else np.nan,
            "max_abundance": summed.max() if len(summed) else np.nan,
            "samples_ge_1pct": int((summed >= args.threshold).sum()),
            "core_ge_1pct_all_samples": bool(np.all(summed >= args.threshold)),
            "median_effective_MAG_number": float(np.nanmedian(eff)) if len(eff) else np.nan,
        })
